...
# Streamlit UI 설정
st.title('문장유사도평균에 따른 이야기 선택')

# ---------- 데이터 불러오기 파트 -----------------
# session_state에 선택한 파일이 없으면 초기값 설정
if 'data_select' not in st.session_state:
    st.session_state['data_select'] = 'c2d2_0924_final.csv'

# 선택한 파일을 업데이트하는 함수
def update_data_select():
    st.session_state['data_select'] = st.session_state['data_select_value']

# 어떤 데이터를 불러오는지 확인할 드롭다운 메뉴 생성 (session_state를 사용하여 선택한 값을 기억)
data_select = st.selectbox(
    '데이터를 선택해주셈', 
    ['c2d2_0924_final.csv', 'annotated_change.csv'], 
    index=['c2d2_0924_final.csv', 'annotated_change.csv'].index(st.session_state['data_select']),
    key='data_select_value',
    on_change=update_data_select
)

# 데이터 불러오기
df = pd.read_csv(f"data/{st.session_state['data_select']}")
...
# 데이터프레임에서 distorted part가 비어있으면 제거, NaN 값 제거, [] 제거
df = df.dropna(subset=['Distorted part'])
df = df[df['Distorted part'].apply(lambda x: len(x) > 0)]

# 드롭다운 메뉴에 표시할 항목 생성 (Distorted part 앞부분 + Thought_문장유사도평균)
df['Dropdown_label'] = df['Distorted part'].apply(lambda x: x[0][:30] + '...' if len(x[0]) > 30 else x[0]) + ' (' + df['Distorted_문장유사도평균'].astype(str) + ')'
...
df_sorted = df.sort_values(by=['Distorted_문장유사도평균', 'Dropdown_label'], ascending=[True, True])

# session_state에 선택한 드롭다운 메뉴가 없으면 초기값 설정
if 'selected_value' not in st.session_state:
    st.session_state['selected_value'] = df_sorted['Dropdown_label'].iloc[0]

# 선택한 항목을 업데이트하는 함수
def update_selected_value():
    st.session_state['selected_value'] = st.session_state['selected_dropdown_value']

# 선택된 값의 인덱스를 가져와서 selectbox에서 그 값이 보이도록 설정
selected_value_index = df_sorted['Dropdown_label'].tolist().index(st.session_state['selected_value'])
print(selected_value_index)

# 드롭다운 메뉴 생성 (session_state를 사용하여 선택한 값을 기억)
selected_value = st.selectbox(
    'Select a story based on Distorted part and Distorted_문장유사도평균:', 
    df_sorted['Dropdown_label'].unique(), 
    index=selected_value_index,  # 현재 선택된 값을 인덱스로 설정
    key='selected_dropdown_value',
    on_change=update_selected_value
)

# 선택한 항목에 해당하는 행 가져오기
selected_row = df_sorted[df_sorted['Dropdown_label'] == st.session_state['selected_value']].iloc[0]
